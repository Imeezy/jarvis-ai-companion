/**
 * CRITICAL FIXES FOR WEB SEARCH INTEGRATION
 * 
 * Key changes:
 * 1. Inject real-time context (time, weather) into ALL system prompts
 * 2. Properly format Anthropic web search tool results
 * 3. Add location context to chat agentic function
 * 4. Skip web search for local queries (time/date)
 * 5. Handle tool results with actual content injection
 */
...
async function lovableCall(system: string, messages: ApiMessage[]): Promise<string> {
  ...
  const j = await r.json();
  return (
    j.choices?.[0]?.message?.content ?? "Anomaly detected, sir."
  );
}
...
export const chatAgentic = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // FIX 1: Always inject real-time context
    const system = await buildEnhancedSystemContext(baseSystem, location);

    // FIX 2: Skip tools for local queries
    if (!useTools || isLocalQuery(messages[messages.length - 1]?.content as string)) {
      try {
